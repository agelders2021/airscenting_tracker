...
class AirScentingUI:
    ...
    def __init__(self):
        ...
        self.splash = SplashScreen(self.root, version="1.0.8-alpha", main_window_geometry=final_geometry)
    ...
    def open_export_dialog(self):
        ...
        if not sv.dog.get():
            self.show_status_message("No Dog Selected", "warning")  # PHASE3A
        ...
        if not trail_maps_folder:
            self.show_status_message("Trail Maps Folder Not Set", "warning")  # PHASE3A
            ...
            if hasattr(self, 'remember_password_var') and sv.remember_password.get():
                ...
                if check_crypto_available():
                    if save_encrypted_password(self.config, db_type, password):
                        ...
                        self.show_status_message(f"Password saved (encrypted) for {db_type}", "info")
                    else:
            ...
            if check_crypto_available():
                ...
                if saved_password:
                    ...
                    if hasattr(sv, 'status'):
                        self.show_status_message(f"Loaded saved password for {db_type}", "info")
                else:
                    sv.db_password.set("")
            else:
                sv.db_password.set("")
