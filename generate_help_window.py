...
class HelpWindow:
    """A searchable help dialog with section navigation."""
    ...
    def setup_ui(self):
        ...
        self.search_var.trace('w', self.filter_sections)
        self.search_entry = ttk.Entry(search_frame, textvariable=self.search_var, width=40)
        self.search_entry.pack(side=tk.LEFT, fill=tk.X, expand=True)
        
        # Paned window for section list and content
        paned = ttk.PanedWindow(main_frame, orient=tk.HORIZONTAL)
        paned.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # Left frame - section list
        left_frame = ttk.Frame(paned)
        paned.add(left_frame, weight=1)
        
        ttk.Label(left_frame, text="Topics:", font=('TkDefaultFont', 10, 'bold')).pack(anchor=tk.W)
